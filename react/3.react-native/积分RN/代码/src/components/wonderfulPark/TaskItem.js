...
import {getReward as getRewardRequest, getVipRewards} from '../../api'
  ...
  return (
    isCompleted(task)
      ? (
        <TouchableHighlight
          {...TOUCH_COLORFUL_MASK}
          style={[styles.task_button, {borderRadius: 5, backgroundColor: '#FF7E00'}]}
          onPress={getReward}
        >
      ...
      : (
        <TouchableHighlight{...TOUCH_LIGHT_MASK} style={[styles.task_button, {borderRadius: 5}]} onPress={handleClick}>
...
export class TaskItem extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      isDone: isDone(task), // 专门针对当前item是否已领取
    }
  }

  render() {
    // area 台湾和大陆区分
    const {task, viplabel, area, showDoubleCard} = this.props
    const {exts, multiple, icon, button} = task
    const notes = exts.filter(e => e.name === 'notes')[0]
    const {animatedValue, isGetScore, isDone: isStateDone} = this.state
    const stateDoneComponent = isStateDone
      ? (
        <View style={[styles.task_button, {borderColor: 'transparent', backgroundColor: '#D8D8D8'}]}>
          <Text style={{fontSize: 14, color: '#ffffff'}}>已完成</Text>
        </View>
      )
      : <CustomButton task={task} area={area} button={button} handleClick={this._handleClick} getReward={this.getReward}/>
    return (
        ...
        <View style={styles.rowContainer}>
          ...
          <View style={styles.task_wrap}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              {!!multiple && !!showDoubleCard && (
                <View style={{marginLeft: 3}}>
                  <GradientView
                    style={{width: 18, height: 14, borderRadius: 3, justifyContent: 'center', alignItems: 'center'}}
                    startColor="#FF3B30"
                    endColor="#FD6540"
                  >
                    <Text style={{fontSize: 10, color: '#ffffff', fontFamily: 'DINAlternate-Bold'}}>x2</Text>
                  </GradientView>
                </View>
              )}
            </View>
            ...
            ? (
              <React.Fragment>
                ...
                <Animated.View style={[styles.animatedScore, {opacity: animatedValue.opacity, top: animatedValue.top}]}>
                  <Text style={{color: '#FF7E00'}}>+{task.score}</Text>
  ...
  _onPress = () => {
    this.props.touchClick(this.props.task)
  }

  _handleClick = () => {
    this.props.handleClick(this.props.task)
  }
  ...
  getReward = () => {
    const {task, from, getData} = this.props
    const {channelCode, score} = task
    if(channelCode) {
      sendClickPingback(PARK_PAGE, 200400, `getreward_${channelCode}`);
    }
