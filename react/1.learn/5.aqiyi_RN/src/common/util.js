...
import {
  ...
  DeviceModule,
  EventModule
} from '@iqiyi/rn-base-modules'
import {isIOS, isObj, isStr} from '@iqiyi/rn-utils'
import {md5} from './md5'
import {isDev, BLOCK_DOWNLOAD_DEVICE_MKEY, COUPON_LIST, DUIBA, GET_ENV, SINGLE_COUPON} from '../constants/configs'
...
export function filterPic(param = [], picType) {
  ...
  for(let i = 0; i < param.length; i++) {
    picList[param[i].photoKey] = param[i].url;
  }
  let result;

  if(picType === undefined) {
    result = picList;
  } else if(picList[picType]) {
    result = picList[picType];
  } else {
    result = picList.default;
  }
...
export function filterExts(list = [], keyName) {
  if(!list) {
    return null
  }
  const extMaps = {}
...
export function goToLogin() { //登录方法
  const params = {
    biz_id: 103,
...
export function blockDownload(list) {
  ...
  if(BLOCK_DOWNLOAD_DEVICE_MKEY.indexOf(mkey) >= 0) {
    return list.filter((item) => {
      ...
      return !item.exts.filter(e => e.name === 'isdownload')[0]
    })
  }
  return list
}
 ...
 * @returns {boolean}
 */
export const isURL = str => (typeof str === 'string'
  ? !!str.trim()
    .match(/^https?/)
  : false)
...
*/
export const formatQipuDataElementsSummary = (item, isSort = true) => {
  let newItem = [];
  ...
  if(item && item.length) {
    ...
    newItem = item.map((m) => {
      const {key_value_pair: values} = m;
      const newValues = {};
      // 将key_value_pair 数组转化为对象，平铺
      values.forEach((keyNames) => {
        newValues[keyNames.name] = keyNames.value;
      });

      if(!newValues.platforms || (newValues.platforms &&
        ((/ios/ig.test(newValues.platforms) && isIOS)
          || (/android/ig.test(newValues.platforms) && !isIOS)))) {
        return {
          ...m,
          values: newValues,
        };
  ...
  // 去掉无意义的空数据
  newItem = newItem.filter(newData => !!newData)
  return newItem;
}
...
FILE = 6;
 */
export function shareSNS(options) {
  const shareInfo = {rpage: 'integral-park', mp_userName: 'gh_ead7a8338ee5', ...options};
  // shareType, 1. 为webpage， 2 标识txt， 5 （小程序？）
  // 兼容测试版小程序, 0 为正式， 1为开发， 2为体验
  if(isDev() && shareInfo.mp_path && shareInfo.shareType === 5) {
    shareInfo.mp_type = 1;
  }
  return QYRNBridge.share(shareInfo);
}
...
// 获取当前页面是否是第一级展示
export function isCurrentPageOnShow({navigation = {}, routeName = ''}) {
  try {
