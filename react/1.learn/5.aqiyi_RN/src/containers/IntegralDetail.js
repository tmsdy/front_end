...
export default class IntegralDetail extends Page {
  ...
  _renderItem(item) {
    ...
    const statusColor =
      status === '+' ? styles.changeTypeOrange : styles.changeTypeGreen;
    const scoreColor =
      status === '+'
        ? {
          color: '#FF6600',
    ...
    return (
      <View style={styles.container}>
        ...
        <WebImage name="jfmx_tbg" style={styles.barTitle}>
          ...
          {!!this.state.lastPeriodScore &&
          <View style={{flexDirection: 'row'}}>
          <Text style={styles.noticeTitle}>{`您${new Date().getFullYear() - 1}年获得的${this.state.lastPeriodScore}积分将于今年6月1日过期`}</Text>
          </View>
          }
        </WebImage>
