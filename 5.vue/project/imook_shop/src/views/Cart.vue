<template>
    <div>
          ...
          <div class="item-list-wrap">
            <div class="cart-item">
              ...
              <ul class="cart-item-list">
                <li v-for="cartItem,index in cartList" :key="cartItem.productId" >
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':cartItem.checked}" @click="editCart('checked',cartItem)">
                    ...
                    <div class="cart-item-pic">
                      <img :src="'/static/'+cartItem.productImg" :alt="cartItem.productName">
                    </div>
                  ...
                  <div class="cart-tab-2">
                    <div class="item-price">{{cartItem.productPrice}}元</div>
                  </div>
                  <div class="cart-tab-3">
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          <a class="input-sub" @click="editCart(0,cartItem)">-</a>
                          <span class="select-ipt">{{cartItem.productNum}}</span>
                          <a class="input-add" @click="editCart(1,cartItem)">+</a>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="cart-tab-4">
                    <div class="item-price-total">{{cartItem.productNum*cartItem.productPrice}}元</div>
                  </div>
                  <div class="cart-tab-5">
                    <div class="cart-item-opration">
                      <a href="javascript:;" class="item-edit-btn" @click="delCartConfirm(cartItem.productId)">
            ...
            <div class="cart-foot-inner">
              ...
              <div class="cart-foot-r">
                <div class="item-total">
                  总计: <span class="total-price">{{totalPrice}}元</span>
                </div>
                <div class="btn-wrap">
                  <a class="btn btn--red">确认支付</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <modal :mdShow="cartShow" @closeModal="closeModal">
        ...
        <div slot="btnGroup">
          <a class="btn btn--m" @click="delCartItem">确认</a>
          <a class="btn btn--m" @click="closeModal">关闭</a>
        </div>
      </modal>
      <modal :mdShow="resultShow" @closeModal="closeModal">
        <p slot="message">{{resultMsg}}</p>
        <div slot="btnGroup">
          <a class="btn btn--m" @click="closeModal">关闭</a>
        </div>
      </modal>
  ...
  import axios from 'axios'
    ...
    export default {
      ...
      methods:{
        ...
        delCartItem(){
          axios.post('/api/goods/cart/del',{productId:this.productId}).then(res=>{
            // console.log(res) ;
            if(res.data.code==0){
        ...
        editCart(flag,item){
            if(flag==1){
              ...
              if(item.productNum<=1){
                axios.post('/api/goods/cart/del',{productId:item.productId}).then(res=>{
                  if(res.data.code==0){
                    this.init() ;
                  }
                })
        ...
        toggleCheckAll(){
          ...
          axios.post("/api/goods/editCheckAll",{checkAll:flag}).then(res=>{
            // console.log(res) ;
          })
        }
      }
