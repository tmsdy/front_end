<template>
    <div class="customerLists">
        ...
        <template v-else>
            ...
            <div :class="isShowCheck?'customerTables MXscroll' : 'customerTablesDetailUse'" :style="{'padding-right': hasScrollbarValue ? '10px' : '15px' }">

                <virtual-list class="MXscroll" :size="64" ref="customerTables" :remain="remain" :bench="remain*2" :item="customerItem" :variable="getVariableHeight" :itemcount="customerListsData.length" :itemprops="getItemProps" />
            </div>
        </template>
    </div>
</template>
...
import virtualList from '@/basecomponents/VirtualList/index.js'
export default {
    name: 'customerLists',
    props: {
        listSet: {
    ...
    created() {
        this.remain = parseInt((this.screenHeight - 410) / 50)
    ...
    methods: {
        ...
        getVariableHeight(itemIndex) {
            return this.customerListsData[itemIndex].title ? 20 : 60
        },
    ...
    watch: {
        screenHeight(height) {
            this.remain = parseInt((height - 410) / 50)
        }
    },
    components: {
        'no-data': NoData,
