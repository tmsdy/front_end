...
fs.readFile(path.join(__dirname, 'ccc.jpg'), (err, buffer) => {

    console.log(Buffer.isBuffer(buffer) && 'readFile 读取图片拿到的是 Buffer 数据')
    // 把读取到的 Buffer 数据，通过 fs writeFile 写入到一个新图片文件中
    fs.writeFile(path.join(__dirname, 'copy.jpg'), buffer, function (err) { })
    ...
    fs.writeFile(path.join(__dirname, 'img_decoded.jpg'), decodedImage, (err) => { })
})