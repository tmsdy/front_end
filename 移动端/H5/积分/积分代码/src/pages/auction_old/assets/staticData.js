...
function getButtonState(cycle, {type, price} = {}) {
  if (cycle === false) {
    if (type === 'day') {
      return getState(false, '今日已获奖，明天再来吧！')
    }
    if (type === 'whole') {
      return getState(false, '抢拍大神，留点给其他小伙伴吧！')
    }
  }

  if (cycle !== 'beforeAuction' && cycle !== 'afterAuction' && type === 'beShotOf') {
  ...
  if (cycle === 'afterAuction') {
    return getState(false, '活动已结束，下次再来')
  }
}
...
const BUTTON = (state) => {
  if (state === 'lifecycle') {
    return {
      beforeAuction: () => getButtonState('beforeAuction'),
