...
export const getLifeCycle = (data, nowTime = Date.now()) => {
  ...
  const nowRound = tempNowRound % roundPeriod === 0
    ? tempNowRound / roundPeriod + 1
    : Math.ceil(tempNowRound / roundPeriod)

  // 当前轮次起始时间
  const currentRoundStartTime = startTime + roundPeriod * (nowRound - 1)
  // 当前轮次计算结果开始时间
  const currentRoundCalcStartTime = startTime + roundPeriod * (nowRound - 1) + bidTime
  // 当前轮次结果公布开始时间
  const currentRoundPublishStartTime = startTime + roundPeriod * (nowRound - 1) + bidTime + calcTime
  // 当前轮次结束时间
  const currentRoundEndTime = startTime + roundPeriod * nowRound
