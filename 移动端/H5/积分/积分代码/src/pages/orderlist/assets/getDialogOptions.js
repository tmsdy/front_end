...
const failReminder = (content = '系统繁忙，请稍后重试') => $.fn.toast({
  content,
  type: 'fail',
  time: 3,
})
  ...
  activeMember(data)
    .then(data => {
      if (data.code === 'A00000') {
        ...
        activeLoading.destroy()
        failReminder(data.msg || data.message)
      }
    })
    .catch(err => {
      activeLoading.destroy()
      failReminder(err.msg || err.message)
    })
}
...
function getDialogButtons({text = null, attrs = {}, clickable = true, onClick = null}) {
  return text ? [{
    className: 'btn-orange-block w500',
    clickable,
    attrs,
    text,
    onClick,
...
function genetateBtnOptionsByCode(order, $dialog) {
  ...
  if (status === 1) {
    ...
    //                   6:'会员直冲', 7:'虚拟积分', 8:'游戏礼包', 9:'黄金会员', 10:'会员直充-激活', 11:'链接发放', 15:'游戏会员'
    ...
    if (type === 4 && subType === 11 && isURL(redeemCodes)) { // 卡券 + 链接发放
      ...
      btnOptions = {
        ...
        onClick: () => {
          ...
          location.href = `/integralh5/orderlist/edit_address?${query}`
        }
      }
    } else if (type === 7 && subType === 15) {
      if (!isIos() && extMap.hasOwnProperty('touse')) {
        btnOptions = {
          text: '去使用',
