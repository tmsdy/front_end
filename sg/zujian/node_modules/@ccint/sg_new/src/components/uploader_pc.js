 ...
 * uploader 文件异步上传
 *
 * IE9 及以下依赖 uploader.swf.js、uploader.swf 实现多文件上传, 其他浏览器原生支持多文件上传
 * IE9 及以下采用 From POST 完成上传, 其他浏览器采用 XMLHttpRequest
 * IE9 及以下不支持拖拽文件上传 (IE9 支持不完整, uploader 限制; IE8 及以下完全不支持)
 * IE9 及以下无法通过 JavaScript 读取文件大小, 需要服务器配合
 * IE 浏览器拖拽上传有一处交互细节与其他浏览器不一致, 详见 releaseCheck
 * 使用拖拽上传需要至少实现 dragenter、drageleave、drop 其中一个回调
 * Safari(Windows) 不支持拖拽上传
 *
 **/
import $ from 'jquery'
import sg from '../sg.js'

sg.Component.define('UploaderPc', {
  properties: {
    url: '',
    maxSize: 0,
    minSize: 0,
    ...
    queuestart: null, // function(list) {}, // multi-file upload queue start, list of filename
    queuecomplete: null, // function() {}, // multi-file upload queue complete
    dragenter: null, // function() {},
    dragleave: null, // function() {},
    drop: null, // function() {},
    dropSelector: null // drop target selector, default element
  },
  template: '<div class="sg_uploader" style="position:absolute;top:0;left:0;width:100%;height:100%;overflow:hidden">' +
  '<ul class="sg_uploader_queue" style="display:none"></ul>' +
  '</div>',
  render: function ($dom) {
    let self = this
    ...
    $dom.append($(self.template))
    self.element = $dom

    self.title && $dom.attr('title', self.title)

    self.uploader = $dom.find('.sg_uploader')
    self._queue = self.uploader.find('.sg_uploader_queue')
    self._extension = (function () {
      if (self.extension && self.extension !== '*') {
        let extension = {}
        $.each(self.extension.split(','), function (index, ext) {
    ...
    let length
    let etag
    let etags = 'click,change,validate,submit,progress,cancel,queuestart,complete,queuecomplete,success,error'.split(',')
    for (i = 0, length = etags.length; i < length; i++) {
      etag = etags[i]
      self['__' + etag] = typeof self[etag] === 'function' ? self[etag] : function () {}
    }
    let drag = {}

    for (let key in self.UPLOADER_DRAG) {
      drag[key] = (function (callback) {
        ...
        return callback
      })(self.UPLOADER_DRAG[key])
    }
    self.drag = self.UPLOADER_DRAG = drag
  ...
  onBeforeDestroy: function (next) {
    ...
    let id
    if (clone) {
      id = clone.attr('data-id')
      if (id) {
        clone.attr('id', id).removeAttr('data-id')
      }
      id = clone.attr('data-position')
      if (id) {
  ...
  upload: function (id) {
    ...
    list = self._queue.children('.uploader_todo')
    list_filename = []
    ...
    if (self._swf) {
      ...
      uploadQueue = function (list) {
        ...
        if (item) {
          ...
          file = item.find('.uploader_file')[0]

          id = index
          name = file.value.substr(file.value.lastIndexOf('\\') + 1)

          if (self.__submit(id, name) === false) {
            return false
          }

          self._submitForm(id, name, file, function (response) {
            self.__complete(id, name, response)
            item.removeClass('uploader_todo')
            uploadQueue(list)
          })
        } else {
          self.__queuecomplete()
        }
      }
      list.find('.uploader_file').each(function () {
        list_filename.push(this.value.substr(this.value.lastIndexOf('\\') + 1))
      })
      if (self.__queuestart(list_filename) === false) {
        self.clearQueue()
        return
      }
      ...
      uploadQueue = function (list) {
        ...
        if (item) {
          ...
          counter = 0
          file = item.find('.uploader_file')[0]
          uploadList(self.toArray(file._dragFiles || file.files))
        } else {
          self.__queuecomplete()
        }
      }
      list.find('.uploader_file').each(function () {
        for (let i = 0, length = this.files.length; i < length; i++) {
          list_filename.push(this.files[i].name)
        }
      })
      if (self.__queuestart(list_filename) === false) {
        self.clearQueue()
  ...
  _submitForm: function (id, name, file, complete) {
    ...
    if (parent.tagName === 'LI') {
      parent = $(parent)
      target = 'uploader_' + (new Date()).getTime()
      ...
      form.append(file).attr('action', url + (~url.indexOf('?') ? '&' : '?') + 'extension=' + self.extension)
      iframe[0].attachEvent('onload', function () {
        if (self.progress) {
          // form 表单提交无法提供文件上传进度, loaded, total 是无效的数字
          self.__progress(id, name, 1, 1, 1)
        }
        complete(iframe[0].contentWindow.document.body.innerHTML.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&'))
      })
    }
  ...
  _submitXhr: function (id, name, file, complete) {
    ...
    if (self.progress) {
      ...
      progress = function (loaded, total) {
        let time = +new Date()
        let percent = parseInt(loaded / total * 100) / 100
        // excute each 100 ms
    ...
    url += (~url.indexOf('?') ? '&' : '?') + 'uploader_file=' + encodeURIComponent(file.name) + '&extension=' + self.extension
  ...
  _createButton: function () {
    ...
    let position = element[0].style.position
    let file
    ...
    if (!window.File && (self.multiple && self.swf && window.SWFUpload)) {
      // require flash
      self.uploader.remove()
      self._createButtonSwf()
    } else {
      // name="uploader_file[]"
      file = $('<input class="uploader_file" name="uploader_file" type="file" tabindex="-1" style="position:absolute;top:0;right:0;bottom:0;font-size:118px;margin:0;padding:0;opacity:0;filter:alpha(opacity=0);cursor:pointer"/>')

      file
        .bind('click', self.events.click)
        .bind('change', self.events.change)
        .attr('accept', getAccept(self.extension))

      if (self.multiple) {
        file.attr('multiple', 'multiple')
      }
      if (self.title) {
        file.attr('title', self.title)
      }
      ...
      if (!self._form && (self.dragenter || self.dragleave || self.drop)) {
        self.drag.init(self)
      }
    }
    ...
    function getAccept (extension) {
      ...
      for (i = 0, length = extension.length; i < length; i++) {
        mime.hasOwnProperty(extension[i]) && accept.push(mime[extension[i]])
      }
      if (accept.length) {
        return accept.join(',')
      }
  ...
  _createButtonSwf: function () {
    ...
    settings = {
      ...
      file_dialog_complete_handler: function (numFilesSelected, numFilesQueued) {
        if (numFilesSelected && self.autoUpload) {
          swf.startUpload()
        }
      },
      upload_start_handler: function (file) {
        let name = file.name
        let id = getFileId(name)
        if (self.__validate(id, name) === false) {
    ...
    if (self.maxSize) {
      settings.file_size_limit = self.maxSize + 'b'
    }

    if (self.extension) {
      settings.file_types = self.extension.replace(/,/g, ';').replace(/(\w+)/g, '*.$1').replace(/^\*$/, '*.*')
    }

    if (self.progress) {
      let precious_time = +new Date()
    ...
    function getFileId (name) {
      if (map_file_id.hasOwnProperty(name)) {
    ...
    self._swf = swf = new SWFUpload(settings) // eslint-disable-line
  ...
  _enQueue: function () {
    ...
    item = $('<li data-index="' + self._index + '" id="uploader_item' + self._index + '" class="uploader_item uploader_todo"></li>')
    queue.append(item)
    item.append(file)
  ...
  _validateFile: function () {
    ...
    function getExtensionError () {
      if (self._extension) {
        ...
        if (!self._extension.hasOwnProperty(ext)) {
  ...
  events: {
    ...
    change: function (e) {
      ...
      if (self.__change(e) === false) {
        self.clearFile()
        return
      }

      if (self._validateFile() === true) {
        self._enQueue()
  ...
  UPLOADER_DRAG: {
    ...
    enter: function (e) {
      ...
      if (window.attachEvent) {
        $(document).bind('mouseover', this.drag.releaseCheck)
      }
      this.drag.trigger(e, 'dragenter')
    },
    over: function (e) {
      e.preventDefault()
    ...
    leave: function (e) {
      if (this.drag.isLeaveDocument(e)) {
        this.drag.release(e, 'dragleave')
      }
    },
    drop: function (e) {
      e.preventDefault()
      this.drag.release(e, 'drop')
    },
    // 释放文件: 拖拽离开网页, 鼠标离开网页, 拖拽放下文件
    release: function (e, etag) {
      document.ondragenter = this.drag.enter
      this.drag.trigger(e, etag || 'mouseleave')
    },
    // IE 拖拽修正: IE 对话框不在最顶层时鼠标离开网页 elementFromPoint 方法没有返回正确的值
    releaseCheck: function (e) {
      if (!e.dataTransfer) {
        $(document).unbind('mouseover', this.drag.releaseCheck)
        this.drag.release(e, 'dragleave')
      }
    },
    isLeaveDocument: window.mozRequestAnimationFrame ? function (e) {
    ...
    dropTarget: window.mozRequestAnimationFrame ? function (e) {
    ...
    trigger: function (e, etag) {
      let widget
      let i
      let list = this.drag.list
      let length = list.length
      let target
      let container

      if (etag === 'drop') {
        target = this.drag.dropTarget(e)
        for (i = 0; i < length; i++) {
          widget = list[i]
          if (widget) {
            if (widget[etag]) {
            ...
            container = widget.dropSelector ? $(widget.dropSelector) : widget.element
