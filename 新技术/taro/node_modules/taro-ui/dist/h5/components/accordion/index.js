...
export default class AtAccordion extends AtComponent {
  ...
  render() {
    ...
    return <View className={classNames('at-accordion', className)} style={customStyle}>
        <View className="at-accordion__header" onClick={this.handleClick.bind(this)}>
          {icon ? <AtIcon customStyle={{
          marginRight: '15px',
          fontSize: `${icon.size || 24}px`
        }} className="at-accordion__icon" value={icon.value} color={icon.color} /> : null}
          <View className="at-accordion__title">
            {title}
          </View>
          <AtIcon customStyle={this.mergeStyle({
          fontSize: '24px'
        }, animatedStyle)} value="chevron-down" color="#c7c7cc" />
        </View>
        <View className={classNames(contentCls)} style={contentStyle}>
          {this.props.children}
        </View>
      </View>;
  }
