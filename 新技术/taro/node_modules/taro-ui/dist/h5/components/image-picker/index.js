...
export default class AtImagePicker extends AtComponent {
  ...
  render() {
    ...
    return <View className={classNames('at-image-picker', className)} style={customStyle}>
        <Input className="at-image-picker__file-input" ref={this.refFileInput} type="file" accept="image/*" multiple={multiple ? 'multiple' : ''} onChange={this.handleImgChoose.bind(this)} />
        {matrix.map((row, i) => <View key={i} className="at-image-picker__flex-box">
              {row.map((item, j) => item.url ? <View className="at-image-picker__flex-item" key={i * length + j}>
                      <View className="at-image-picker__item">
                        <View className="at-image-picker__remove-btn" onClick={this.handleRemoveImg.bind(this, i * length + j)}>
                          <AtIcon customStyle={{ fontSize: '10px' }} value="close" color="#fff" />
                        </View>
                        <Image mode={mode} onClick={this.handleImageClick.bind(this, i * length + j)} className="at-image-picker__preview-img" src={item.url} />
                      </View>
                    </View> : item.type === 'blank' ? <View className="at-image-picker__flex-item" key={item}>
                      </View> : <View className="at-image-picker__flex-item" onClick={this.chooseFile.bind(this)}>
