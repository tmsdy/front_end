...
export default class AtIndexes extends AtComponent {
  ...
  render() {
    ...
    return <View className={classNames({
      'at-indexes': true
    }, className)} style={customStyle} ref={this.getIndexesRef}>
        <View className="at-indexes__menu" onTouchMove={this.handleTouchMove.bind(this)} onTouchEnd={this.handleTouchEnd.bind(this)} ref={this.getMenuRef}>
          <View className="at-indexes__menu-item" onClick={this.jumpTarget.bind(this, 'at-indexes__top', 0)}>
            {topKey}
          </View>
          {list.map((item, i) => <View key={item.key} className="at-indexes__menu-item" onClick={this.jumpTarget.bind(this, `at-indexes__list-${item.key}`, i + 1)}>
                {item.key}
              </View>)}
        </View>
        <ScrollView className="at-indexes__body" scrollY scrollWithAnimation={animation} scrollTop={this.state.scrollTop} scrollIntoView={this.state.targetView} onScroll={this.handleScroll.bind(this)} ref={this.getListRef}>
          <View className="at-indexes__content" id="at-indexes__top">
            {this.props.children}
          </View>
          {list.map(listItem => <View key={listItem.key} className="at-indexes__list">
                <View id={`at-indexes__list-${listItem.key}`} className="at-indexes__list-title">
                  {listItem.title}
                </View>
                <AtList>
                  {listItem.items && listItem.items.map(item => <AtListItem key={item.name} title={item.name} onClick={this.handleClick.bind(this, item)} />)}
                </AtList>
              </View>)}
        </ScrollView>
      </View>;
