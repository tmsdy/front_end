...
export default class AtSwipeAction extends AtComponent {
  ...
  componentDidMount() {
    delayQuerySelector(Taro.getEnv() === Taro.ENV_TYPE.WEB ? this : this.$scope, '.at-swipe-action').then(res => {
      this.domInfo = res[0];
    });
  }
  ...
  render() {
    ...
    return <View className={rootClass} onTouchMove={this.handleTouchMove} onTouchEnd={this.handleTouchEnd} onTouchStart={this.handleTouchStart}>
        ...
        {Array.isArray(options) && options.length > 0 ? <AtSwipeActionOptions onQueryedDom={this.handleDomInfo}>
