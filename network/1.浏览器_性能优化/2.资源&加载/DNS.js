/*
* 1.为什么需要DNS系统？
    1）我们输入的是域名，而数据包是通过IP地址传给对方的。DNS系统可以将域名和IP地址映射，得到IP的过程就是DNS解析。
    2）浏览器提供了DNS数据缓存功能。如果一个域名已经解析过，那会把解析的结果缓存下来，下次处理直接走缓存，不需要经过 DNS解析。默认是80端口

* 2.DNS系统: 是一个分布式数据库，功能是联系域名和ip地址。域名与ip的对应关系,可分为各种类型
