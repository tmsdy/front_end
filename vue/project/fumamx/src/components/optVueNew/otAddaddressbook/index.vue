<template>
    <!-- 加入地址簿 -->
    <el-dialog v-dialogDrag :title="$t('mxpcweb.client.list.1542173234808')" :visible.sync="dialog" :closeOnClickModal="false" custom-class="width620" :modal-append-to-body="false">
        <div class="addLowerCust" v-show="tabData == '1'">
            <div style="padding-left:20px">
                <!-- 全选 -->
                <el-checkbox style="margin-left: -9px;margin-bottom: 20px;" :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">{{$t('mxpcweb.client.list.1542173236960')}}</el-checkbox>
                <!-- 选中所有联系人首个邮箱 -->
            ...
            <div class="mailBox MXscroll">
                <el-checkbox-group v-model="checkedCities1" @change="handleCheckedCitiesChange">
                    ...
                    <el-row class="contentsBox" :gutter="20" v-for="(itemList,itemListIndex) in custContacts" :key="itemListIndex">
                        ...
                        <el-col :span="18">
                            <el-row :gutter="20" v-for="(item,index) in itemList.custContacts" v-if="item.mailAddress&&item.mailAddress.length!==0" :key="index">
        ...
        <div class="addLowerCust" v-show="tabData == '2'">
            <div class="mailBox MXscroll">
                <address-list v-if="dialog" ref="addressList"></address-list>
            </div>
        </div>
        <div v-if="tabData == '1'" slot="footer" class="dialogFooter" style="text-align:center">
            ...
            <!-- 下一步 -->
            <el-button type="primary" @click="tabData = '2'">{{$t('mxpcweb.client.list.1542173237736')}}</el-button>
        </div>
        <div v-else slot="footer" class="dialogFooter" style="text-align:center">
            <!-- 上一步 -->
            <el-button @click="tabData = '1'">{{$t('mxpcweb.client.list.1542173238840')}}</el-button>
            <!-- 确定 -->
            <el-button type="primary" @click="submit" :loading="subLoading">{{$t('mxpcweb.client.list.1542173184801')}}</el-button>
        </div>
    </el-dialog>
</template>
<script>
...
export default {
    ...
    methods: {
        submit() {
            ...
            this.$refs.addressList.submit(this.returnMailList(this.checkedCities1)).then(() => {
        ...
        openWindow(obj) {
            ...
            // if (_this.$refs.addressList) {
            //     _this.$refs.addressList.clearData()
            // }
            _this.tabData = '1'
            _this.dialog = true
        },
