<template>
<div class="DescriptionBox" :style="{'padding-left': labelWidth}">
    <div v-if="showLabel" class="labelWidth" :class="prop()?'label':''" :style="'width:' + labelWidth">{{itemData.cnFieldCaption}}</div>
    <div class="Description text-hover" @click="dialogOpen()"  :style="{width: rightWidth}">
        {{text}}
    </div>
    ...
    methods: {
        ...
        prop () {
            ...
            if (this.isProp == true) {
                ...
                if (this.itemData.isNotNull == 1 && JSON.stringify(this.DescriptionKey) == '{}') {
                    return false
                } else {
                    return true
                }
