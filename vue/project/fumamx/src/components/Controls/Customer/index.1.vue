    ...
    <el-form :model="ruleForm" onsubmit="return false" :rules="rules" ref="ruleForm" :label-width="labelWidth" :label-position="labelPosition">
        <el-form-item :label="showLabel?itemData.cnFieldCaption:''" :prop="prop()" :style="{'margin-bottom':isProp?0:''}">
            <el-select clearable :disabled="itemData.disabled" v-model="ruleForm.input" @visible-change="click" :placeholder="itemData.cnFieldHint" :maxlength="itemData.fieldLength" @change="change" :size="size" :style="{width: rightWidth}">
              <div class="searchBox">
                <el-input v-model="keyWords" ref="searchInput" @change="getData(keyWords)" icon="search" :size="size"></el-input>
              </div>
              <div class="customer MXscroll">
                <el-option v-for="(item, index) in hideList" :key="index" v-show="false" :label="item.custName" :value="item.custId+''"></el-option>
...
export default {
  ...
  methods: {
      ...
      if (items && items != '') {
        if (isObject(items)) {
          ...
          this.options.forEach(element => {
            if (element.custId == items.custId) {
              isHave = true
            }
          })
          ...
          if (isHave) {
            this.ruleForm.input = items.custId + ''
          } else {
          ...
          let isHave = false
          this.options.forEach(element => {
            if (items == element.custId) {
              isHave = true
            }
          })
          this.hideList.forEach(element => {
            if (element.custId == items) {
              isHave = true
            }
          })
          if (isHave) {
    ...
    change () { // 数据改变重置分页条数，提交数据
      ...
      let isHave = false
      this.options.forEach(element => {
        if (newValue == element.custId) {
          items = element
          isHave = true
        }
      })
      if (!isHave && newValue != '') {
        this.updatas1({custId: newValue})
      }
      this.$emit('custChange', items)
      if (this.isLowerCusts) { // 判断父组件是否下级客户需求
        let lowerCust = {}
        this.options.forEach((item) => { // 遍历客户列表判断当前选中的客户是哪条数据
          if (this.ruleForm.input && this.ruleForm.input == item.custId + '') {
            lowerCust = item
            return false
          }
        })
