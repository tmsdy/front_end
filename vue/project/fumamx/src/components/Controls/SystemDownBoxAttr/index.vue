<template>
    <el-form :model="ruleForm" onsubmit="return false" :rules="rules" ref="ruleForm" :label-width="labelWidth" :label-position="labelPosition">
        <el-form-item :label="showLabel?itemData.cnFieldCaption:''" :prop="prop()" :style="{'margin-bottom':isProp?0:''}">
          <drop-down ref="dropDown" labelKey="itemName" valueKey="dictItemCode" inUse :itemData="itemData" @change="change" :ruleForm="ruleForm" :list="itemData.dictItems" :rightWidth="rightWidth" :size="size"></drop-down>
        </el-form-item>
    </el-form>
</template>
...
export default {
  ...
  methods: {
    getData () {
      let options = []
      if (this.itemData.dictItems instanceof Array) {
        this.itemData.dictItems.forEach(element => {
