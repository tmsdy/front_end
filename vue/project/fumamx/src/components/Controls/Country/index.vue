<template>
    <el-form :model="ruleForm" onsubmit="return false" :rules="rules" ref="ruleForm" :label-width="labelWidth" :label-position="labelPosition">
        <el-form-item :label="showLabel?itemData.cnFieldCaption:''" :prop="prop()" :style="{'margin-bottom':isProp?0:''}">
          <drop-down ref="dropDown" labelKey="cnName" valueKey="countryId" :itemData="itemData" @change="change" :ruleForm="ruleForm" :list="options" :rightWidth="rightWidth" :size="size"></drop-down>
        </el-form-item>
    </el-form>
</template>
  ...
  created () {
    if (this.ruleForm.input != '') {
      let value = {
        ...
        value: this.ruleForm.input == -1 ? '' : this.ruleForm.input
      }
