<template>
    <div v-show="PreviewVisible">
        <!-- 文件预览 -->
        <el-dialog :title="$t('mxpcweb.document.global.1529991056048')" :visible.sync="PreviewVisible" top="5%" size="full" custom-class="previewBox" :modal-append-to-body="false">
            <!-- 关闭预览 -->
            ...
            <div v-if="PreviewVisible" class="box">
                ...
                <template v-if="contentShow">
                    ...
                    <!-- office  xml文档-->
                    <transition name="el-fade-in-linear">
                        <iframe class="officeBox MXscroll" v-if="hasExtName(typeMap.officeArr)" :src='url' frameborder='0'>
                        </iframe>
                    </transition>

                    <!-- xml文档-->
                    <transition name="el-fade-in-linear">
                        <iframe ref="xmliframe" @load="loadedContent" src="/static/preview/content.html" class="xmlBox MXscroll" v-if="hasExtName(typeMap.xmlArr)" frameborder='0'>
                        </iframe>
                    </transition>

                    <!-- eml邮件-->
                    <transition name="el-fade-in-linear">
                        <eml class="emlBox MXscroll" v-if="hasExtName(typeMap.emlArr)" :data="textContent">
                        </eml>
                    </transition>
...
export default {
    ...
    methods: {
        ...
        zoom(event) {
            ...
            if (this.isZoom) {
                let x = event.clientX - img.offset().left
                let y = event.clientY - img.offset().top
        ...
        show(data, index, mId) {
            ...
            this.canShowArr = [].concat(...Object.values(this.typeMap))
            this.returnClose()
            this.viewInit()
        },
        open(dataArr, index) {
        ...
        loadedContent() {
            ...
            try {
                const vm = this.$refs.xmliframe.contentWindow.vm
                vm.showContent(this.textContent)
            } catch (err) {
        ...
        goPreview(_url) {
            ...
            if (this.hasExtName(this.typeMap.imgArr)) {
                ...
                img.onerror = function () {
                    _this.url = _this.Global.errorImg
                    img = null
                    _this.isLoading = false
                    _this.isError = true
                }
                ...
                // _this.url = `http://192.168.8.167:8012/onlinePreview?url=${_url}`
                _this.url = `https://view.officeapps.live.com/op/embed.aspx?src=${_url}&wdStartOn=1&wdPrint=1&wdEmbedCode=1`
                _this.isLoading = false
        ...
        pdfView(src) {
            let durl = `${this.Global.baseURL}${this.Global.api.Files.all_pdf}?src=${encodeURIComponent(src)}`
            this.url = '/static/pdf/web/viewer.html?file=' + encodeURIComponent(durl)
            this.isLoading = false
        }
    },
    components: {
