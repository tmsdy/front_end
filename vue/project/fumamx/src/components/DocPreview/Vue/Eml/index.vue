<template>
    <div v-if="data" class="mailDetail MXscroll" ref="mailDetail">
        ...
        <div class="itemTitle">
            ...
            <div class="timeMore">{{DateDisplay(data.sentDate)}}</div>
        ...
        <div v-if="data.containAttachment" class="divAttch">
            <div class="item ">
                ...
                <span v-if="data.containAttachment">{{attachmentCount}} 个</span>&nbsp;
                <span class="text-blue text-hover" @click="getDownload(data.mId,'all',0)" v-if="attachmentList.length>0">{{$t('mxpcweb.mail.1528701859716')}}</span>
                <template v-if="isMailFile">
                    <ul class="file">
                        <li v-for="(item10,index10) in attachmentList" :key="index10">
                            <div class="ico">
                            ...
                            <div class="view">
                                <span @click.stop="$refs.filePreview.show(attachmentList,index10)">
        ...
        <iframe ref="iframe" src="/static/mail/content.html" @load="loadedContent" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" width="100%" height="100%"></iframe>
...
export default {
    ...
    methods: {
        ...
        DateDisplay(times) {
            ...
            var Temp = times.split(' ')
            var formatStr = ''
            if (Temp[0].indexOf('年') != -1) { // 存在中文日期
                formatStr = Temp[0].substring(0, 4) + '-' + Temp[0].substring(5, 7) + '-' + Temp[0].substring(8, 10) + ' ' + Temp[2]
                return this.$m_formulateTime(formatStr)
            }
            var timArry = times.split('-')
            if (timArry[0] != curData.getFullYear()) { // 年份不同返回年
                return timArry[0]
            } else {
                formatStr = Temp[0] + ' ' + Temp[2]
                return this.$m_formulateTime(formatStr)
            }
        },
        ...
        loadedContent() {
            const _this = this
            if (_this.data.htmlContent == '' || _this.data.htmlContent == undefined) {
                return
            }
            try {
                const vm = _this.$refs.iframe.contentWindow.vm
                vm.showContent(_this.data.htmlContent, (h) => {
                    _this.$refs.iframe.style.height = h + 'px' // 文档高设置到 iframe
                })
            } catch (err) {
