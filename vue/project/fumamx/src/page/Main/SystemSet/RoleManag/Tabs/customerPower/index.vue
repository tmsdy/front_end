<template>
    <div class="customerPower MXscroll">
        ...
        <el-form v-else :model="ruleForm" ref="ruleForm">
            <table class="table">
                ...
                <tbody>
                    <div v-for="(item,index) in ruleForm.modules2" :key="index">
                        <tr v-if="item.moduleCode == tabModuleNum">
                            <td :colspan="index">
                        ...
                        <tr v-if="item.moduleCode == tabModuleNum">
                            <td>
                                <table class="tableSub">
                                    <tr>
                                        <!-- 范围 -->
                                        <th width="133">{{$t('mxpcweb.systemset.rolemanag.1530596213845')}}</th>
                                        <th width="70" v-for="(item2,index2) in item.functions" :key="index2" v-if="item2.funType != 6" class="text-center">{{item2.text}}</th>
                                        <th></th>
                                    </tr>
                                    <tr>
                                        <td>
                                            <div class="radio_item">
                                                ...
                                                <span class="deptRadioName">
                                                    <!-- 指定部门 -->{{$t('mxpcweb.systemset.rolemanag.1530597475707')}}
                                                </span>
                                            </div>
                                        </td>
                                        <td v-for="(item2,index2) in item.functions" :key="index2" v-if="item2.funType != 6" class="text-center">
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="7">
                                            <ul style="margin-left:10px; line-height:25px;">
                                                <li v-for="(item2,index2) in checkReturn(item.functions)" :key="index2" v-if="item2.funType == 6">
                                                    <el-checkbox v-model="item2.rightValue">{{item2.text}}</el-checkbox>
                                                </li>
                                            </ul>
                                        </td>
                                        <td></td>
                    ...
                    <tr>
                        <td>
                            <table class="tableSub" style="margin-bottom:6px;">
                                <tr>
                                    <td width="133">
                                    ...
                                    <td style="line-height:25px;">
                                        ...
                                        <div v-for="(item,index) in selectedFields" v-if="item.fieldName != ''" :key="index">
                                            <span class="text-red">{{item.cnFieldCaption}}</span>
                                            <!-- 为 -->{{$t('mxpcweb.systemset.rolemanag.1530599460283')}}
                                            <span v-for="(item2,index2) in item.dictItemsValues" :key="index2">
                                                <span v-for="(item3,index3) in item.downValues" :key="index3" v-if="item2 == item3.dictItemCode" class="text-yellow">
                                                    {{item3.itemName}}{{item.dictItemsValues.length-1 !== index2 ? '、' : ''}}
                                                </span>
                                            </span>
                                            <!-- 并且 -->
                                            <span style="padding:0 12px;">{{selectedFields.length-2 !== index ? $t('mxpcweb.systemset.rolemanag.1530599508868') : ''}}</span>
                                        </div>
                        ...
                        <td>
                            <table class="tableSub">
                                <tr>
                                    ...
                                    <td style="line-height:25px;">
                                        <el-button :plain="true" type="info" @click="$refs.DialogFieldPower.isOpen(tabModuleNum)" style="padding-left:7px; padding-right:8px;">
            ...
            <table class="table">
                <caption>
                    <!-- 当用户为客户的拥有人时对此客户的相关资料的操作权限 -->{{$t('mxpcweb.systemset.rolemanag.1530695972666')}}
                </caption>
                <tbody>
                    <div v-for="(item,index) in ruleForm.modules2" :key="index">
                        <tr v-if="item.moduleCode != tabModuleNum">
                            ...
                            <td>
                                <table class="tableSub">
                                    <tr>
                                        ...
                                        <th width="70" v-for="(item2,index2) in item.functions" :key="index2" class="text-center">{{item2.text}}</th>
                                        <th></th>
                                    </tr>
                                    <tr>
                                        <td>
                                        ...
                                        <td v-for="(item2,index2) in item.functions" :key="index2" class="text-center">
                                            <el-radio-group v-model="item2.rightValue">
                                                <el-radio label="0"> &nbsp; </el-radio>
                                                <el-radio label="1"> &nbsp; </el-radio>
                                                <el-radio label="2"> &nbsp; </el-radio>
                        ...
                        <hr :key="index" v-if="item.moduleCode != tabModuleNum">
                    </div>
                </tbody>
            </table>
    ...
    methods: {
        ...
        getData() {
            ...
            Promise.all([p1, p2, p3, p4, p5]).then(function (results) {
                ...
                //     return item.moduleCode == 'BF001';
                // });
                // // console.log(fieldRightsCust)

                // results[5].forEach(item => {
                //     //默认定个权限
                //     item.showFlag = true;
                //     item.editState = 1;
        ...
        submit() {
            ...
            let arrDataValue = []
            let isSubmit = true
            let newObjArr = JSON.parse(JSON.stringify(_this.ruleForm.modules2))
            newObjArr.forEach(function (item) {
                item.functions.forEach(function (item2) {
                    ...
                    if (item2.rightValue == '32') {
                        ...
                        // funType = 6 的选项 转换值 提交
                        if (item2.funType == 6) {
                            item2.rightValue = item2.rightValue ? '1' : '0'
                        }
