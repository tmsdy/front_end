    ...
    <div class="customerPower MXscroll">
        ...
        <el-form v-else :model="ruleForm" ref="ruleForm">
            <table class="table">
                ...
                <tbody>
                    <div v-for="(item,index) in ruleForm.modules2" :key="index">
                        ...
                        <tr v-if="item.moduleCode == tabModuleNum">
                            <!--<td width="80" style="color:#555;" :rowspan="index+3">{{item.moduleName}}</td>-->
                            <td>
                                ...
                                <tr>
                                    ...
                                    <td style="line-height:25px;">
                                        ...
                                        <div v-for="(item,index) in selectedFields" v-if="item.fieldName != ''" :key="index">
                                            ...
                                            {{$t('mxpcweb.systemset.rolemanag.1530599460283')}}
            ...
            <table class="table">
                <caption>
                    <!-- 当客户在公海时相关资料查看权限 -->
                    {{$t('mxpcweb.systemset.rolemanag.1530597734921')}}
                </caption>
                <tbody>
                    <div v-for="(item,index) in ruleForm.modules2" :key="index">
                        <tr v-if="item.moduleCode != tabModuleNum">
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            if (!isSubmit) {
                /* '指定部门项请选择部门！' */
                _this.$message(_this.$t('mxpcweb.systemset.rolemanag.1530597065963'))
                return
            }
            _this.$http.post(_this.Global.baseURL + _this.Global.api.SystemSet.rolemanag.permissionsPut, data).then(function (res) {
                if (res.body.code.toString() == _this.Global.RES_OK) {
                    /* 保存成功 */
                    _this.$message.success(_this.$t('mxpcweb.systemset.rolemanag.1530596143028'))
                } else {
                    _this.$message.error(_this.msg(res.body))
                }
            }, function (res) {
