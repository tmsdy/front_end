<template>
    <div class="BaseMsg">
        <!-- 基本信息 -->
        <p class="contactTitle">{{$t('mxpcweb.sale.components.1557565147725')}}</p>
        <div class="listBox">
                        ...
                        <el-row v-if="item.fieldGroup == 0">
                            <el-col :span="24">
                                <div v-if="item.controlTypeId == 4||item.controlTypeId == 15||item.controlTypeId == 46" style="margin-left:16px;">
                                    <component size="small" :nameId="item.fieldName" :optCode="optCode" :moduleCode="moduleCode" :dataId="'control'+index" @relyOn="relyOn" class="component" labelWidth="120px" v-bind:is="'control'+item.controlTypeId" @closeWindow="closeWindow" :checkRepeat="true" ref="control" :itemData="item" rightWidth="672px"  :controlData.sync="item.controlData" labelPosition="left" @returnFieldList="returnFieldList"></component>
                                </div>
                                <component size="small" @themeChange="themeChange" @custChange="custChange" @CurrencyChange="CurrencyChange" v-else :nameId="item.fieldName" :optCode="optCode" :moduleCode="moduleCode" :dataId="'control'+index"  @relyOn="relyOn" style="margin-left:16px;" labelWidth="120px" class="component" v-bind:is="'control'+item.controlTypeId" @closeWindow="closeWindow" :checkRepeat="true" ref="control" :itemData="item" rightWidth="240px"  :controlData.sync="item.controlData" labelPosition="left" @returnFieldList="returnFieldList"></component>
                            </el-col>
                        </el-row>
                        <el-row v-else>
                            <component size="small" :nameId="item.fieldName" @relyOn="relyOn" @returnFieldList="returnFieldList" :optCode="optCode" :moduleCode="moduleCode" :dataId="'control'+index" v-if="item.fieldGroup==1" labelWidth="120px" style="margin-left:16px;" class="component" v-bind:is="'group'+item.fieldGroup"  ref="control" :itemData="item"  rightWidth="672px" rightWidth1="165px" labelPosition="left"></component>
                            <component size="small"  :optCode="optCode" @returnFieldList="returnFieldList" :moduleCode="moduleCode" :nameId="item.fieldName" @relyOn="relyOn" :dataId="'control'+index" v-else class="component" labelWidth="120px" style="margin-left:16px;" v-bind:is="'group'+item.fieldGroup"  ref="control" :itemData="item"   rightWidth="315px" rightWidth1="118px" labelPosition="left"></component>
                        </el-row>
                    </div>
                </template>
            </template>
                ...
                if (element.fieldCategory != 4) {
                    if (element.fieldGroup == 0) {
                        if (element.controlData != this.itemData[element.fieldName] && JSON.stringify(element.controlData) != JSON.stringify(this.itemData[element.fieldName])) {
                            this.addPeopleFrom[element.fieldName] = element.controlData != '' ? element.controlData : 0
                        } else if (this.addPeopleFrom[element.fieldName]) {
                            delete this.addPeopleFrom[element.fieldName]
                        }
                    } else {
                        element.group.forEach((item) => {
                            ...
                            if (JSON.stringify(item.controlData) != JSON.stringify(this.itemData[item.fieldName])) {
                                this.addPeopleFrom[item.fieldName] = item.controlData ? element.controlData : 0
                            } else if (this.addPeopleFrom[item.fieldName]) {
                                delete this.addPeopleFrom[item.fieldName]
                            }
                        })
