...
<div class="HistoricalQuotation">
    <div class="recent">
        ...
        <div class="maxBox">
            ...
            <div class="price">{{returnCurrency(maxQuotation.strucId_1)}}{{maxQuotation.quotedPrice || '-.-'}}</div>
            <div class="date">({{maxQuotation.strucId_1 && maxQuotation.strucId_1.quotaDate ? maxQuotation.strucId_1.quotaDate.slice(0, 10) : '----/--/--'}})</div>
        </div>
        <div class="minBox">
            <!-- 最低报价 -->
            <div class="title">{{$t('mxpcweb.sale.components.1557565407892')}}</div>
            <div class="price">{{returnCurrency(minQuotation.strucId_1)}}{{minQuotation.quotedPrice || '-.-'}}</div>
            <div class="date">({{minQuotation.strucId_1 && minQuotation.strucId_1.quotaDate ? minQuotation.strucId_1.quotaDate.slice(0, 10) : '----/--/--'}})</div>
        </div>
    </div>
    <div class="searchBox">
        <div class="time">
    ...
    <div class="listBox">
        <div class="title">
            <el-row>
                <el-col :span="9">
                    <!-- 报价单编号 -->
                    <div>{{$t('mxpcweb.sale.components.1557565415740')}}</div>
                </el-col>
                <el-col :span="9">
                    <!-- 报价日期 -->
                    <div>{{$t('mxpcweb.sale.components.1557565264648')}}</div>
                </el-col>
                <el-col :span="6">
                    <!-- 单价/数量 -->
        ...
        <template v-if="list.length > 0">
            <template v-for="(item, index) in list">
                <div class="list" :key="index" v-if="item.strucId_1">
                    <el-row>
                        <el-col :span="9">
                            <div v-if="item.strucId_1.quotaCode" class="code text-hover" @click="toDetail(item)">{{item.strucId_1.quotaCode}}</div>
                            <div v-else class="code">-</div>
                        </el-col>
                        <el-col :span="9">
                            <div>{{item.strucId_1.quotaDate ? item.strucId_1.quotaDate.slice(0, 10) : '&nbsp;'}}</div>
                            <div>{{contactList[item.strucId_1.ownerCtId] || '&nbsp;'}}</div>
                        </el-col>
                        <el-col :span="6">
                            <div>{{returnCurrency(item.strucId_1) || ''}}{{item.quotedPrice || '&nbsp;'}}</div>
