<template>
    <div class="GoodsMsg">
                ...
                <div class="floatRight MXscroll">
                    <table style="min-width:1110px;border-color:rgba(192,196,204,1)" align="center">
                        ...
                        <tr class="list" v-for="(item, index) in list" :key="index" :class="(chekckItem == index + 1) || (hoverItem == index + 1) ? 'list1' : ''" @mouseenter="hoverItem = index + 1" @mouseleave="hoverItem = ''">
                            ...
                            <td><span v-if="item.spuCode">{{item.salePrice}}</span></td>
                            <td>
                                <!-- 请输入 -->
                                <template v-if="item.spuCode">
                                    <input @click.stop :placeholder="$t('mxpcweb.sale.components.1557565182681')" type="number" v-model="item.quotedPrice"
                                    min="0"
        ...
        <goods-view ref="goodsView" :catgName="catgName" :categoryList="categoryList" :goodsType="goodsType" @checkGoodsList="checkGoodsList"></goods-view>
        <img-set ref="imgSet" @updateImg="updateImg"></img-set>
        <ot-edit ref="otEdit" :moduleCode="moduleCode" @otEditList="otEditList"></ot-edit>
...
export default {
    ...
    methods: {
        ...
        itemChange(index, type) {
            let { saleQty, quotedPrice, offType, offAmt, offRt, unitQty, unitNW, unitGW, unitVol, boxQty } = this.list[index]
            ...
            if (offType == '2') {
                this.list[index].offAmt = parseFloat((saleQty * quotedPrice * offRt / 100).toFixed(2))
            } else if (offType == '1') {
                this.list[index].offRt = (saleQty * quotedPrice) ? parseFloat(((offAmt / (saleQty * quotedPrice)) * 100).toFixed(2)) : 0
            }
