        ...
        <el-dialog v-dialogDrag :title="$t('mxpcweb.mail.1528955361707')" :visible.sync="isOpen" custom-class="width620" @close="resetForm('dialogForm')" :modal-append-to-body="false" :close-on-click-modal="false">
            <el-form :model="dialog" :rules="dialogRules" ref="dialogForm" label-width="95px">
                ...
                <!--<el-form-item label="处理" prop="" class="marginTop-10">
                    <div>
                        <el-checkbox v-model="dialog.isLabelDistributed">标记为已分发</el-checkbox>
                    </div>
                    <div>
                        <el-checkbox v-model="dialog.isAutoDistributed">发件人"vibowl@fumasoft.com"以后再来信自动分发给以上人员</el-checkbox>
                    </div>
                </el-form-item> -->
                <!-- 提醒@ -->
...
export default {
    ...
    methods: {
        ...
        // 父组件来调用
        isDialog(to, mailList) {
            this.mailIds = ''
            if (to == 'open') {
                if (isArray(mailList)) {
                    mailList.forEach(element => {
                        this.mailIds = this.mailIds + element + ','
                    })
        ...
        submit(formName) {
            ...
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    _this.$http.post(_this.Global.baseURL + _this.Global.api.Mail.mailDistributePost, data)
                        .then(
                            function (res) {
