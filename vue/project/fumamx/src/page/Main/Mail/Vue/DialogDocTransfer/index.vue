<template>
    <div class="DialogDocTransfer">
        <!-- 选择目录 -->
        <el-dialog :title="$t('mxpcweb.document.global.1532682773083')" :visible.sync="isOpen" :close-on-click-modal="false"  v-dialogDrag>
            <div class="tabBox">
                <ul class="clearfix">
            ...
            <div slot="footer" class="dialog-footer clearfix">
                ...
                <el-button :disabled="isKRoot||noPower" :loading="isSaving" type="primary" @click="commit()">
                    {{$t('mxpcweb.document.global.1532682728459',[navList.name[navList.name.length-1]])}}
                </el-button>
...
import { byteCalc, setFileIcon, hasImage } from '@/page/Main/Document/Vue/docUtils.js'
export default {
    name: 'DialogDocTransfer',
    props: {},
    data() {
        return {
            isSaving: false,
    ...
    methods: {
        ...
        open(attachmentList = []) {
            ...
            this.fileList = attachmentList.map(item => {
                return { fileName: encodeURI(item.name), fileSize: item.size * 1024, downloadUrl: item.url }
            })
        ...
        async commit() {
            this.isSaving = true
            let url = this.Global.baseURL + this.Global.api.v2.folders_transferFiles
            let data = {
            ...
            try {
                ...
                if (res.body.code.toString() === this.Global.RES_OK) {
                    /* 转存成功 */
                    this.$message.success(this.$t('mxpcweb.document.global.1532682808402'))
                    // this.getData()
                    this.isOpen = false
                } else {
