<template>
    <div class="ListCenter" :style="{width:( custId==''&&mailAddress==''?'380px':'100%')}">
        ...
        <template v-if="custId==''&&mailAddress==''">
            <!-- 邮件筛选，组件 -->
            <mail-filter ref="mail-filter" v-on:mailListHeight="mailListHeight" @filterGetMail="filterGetMail" :dynamicTags="dynamicTags" @OpenAdvancedSearch="$refs.AdvancedSearch.isDialog('open')" @SearchClick="SearchClick"></mail-filter>
        </template>
        ...
        <el-checkbox-group v-model="checkedListAll" @change="handleCheckedCitiesChange">
            <div class="mailList MXscroll" ref="mailList" v-loading="loading" :style="{'padding-bottom':(custId==''&&mailAddress==''?'25px':'42px')}">
                <dl v-for="(item,index) in listCenterData" :key="index">
                    ...
                    <dd :class="{'new':(item2.status==-1),'old':(item2.status!=-1),'active':(index==checkgroupIndex&&index2==checkedIndex&&custId==''&&mailAddress=='')}" v-for="(item2,index2) in item.list" :key="index2" @click="ListCenterShowDetail(item2,index,index2)" @dblclick="fullScreen(item2.mId,item2.subject,item2.source,item2.folder)">
                        <template v-if="custId!=''||mailAddress!=''">
                            <p class="tag" :style="{background:(item2.source!=undefined?'#00C2B3':'#D0021B')}">
                                <span class="text">
                                    <!-- {{item.source}} -->
                                    {{item2.source!=undefined?'发送':'接收'}}
                                </span></p>
                        </template>

                        <aside v-if="SearchSelect!='c'">
                            <single-operation ref="SingleOperation" :DetailData="item2" :ListDetail="item" @OperationTrigger="OperationTrigger"></single-operation>
                        </aside>
                        <div class="leftBtns">
                            <div :class="checkedListAll.length==0?'checkIt':'checkIt2'">
                                <i v-if="item2.stickyFlag==1&&checkedListAll.length==0" class="tagStar el-icon-star-on"></i>
                                <span @click.stop="" v-if="custId==''&&mailAddress==''">
                                    <el-checkbox :label="item2.mId">{{''}}</el-checkbox>
                                </span>
                            </div>
                            <div v-if="item2.comments" class="dotClass" :style="setCommentsColor(item2.comments)" :title="getCommentsConten(item2.comments)" @click.stop="$emit('OperationTrigger', 'handleCommand',{actionName:'Comment',mId:item2.mId,subject:item2.subject})">
                                <i class="iconfont icon-dot"></i>
                            </div>
                            <div class="dotClass" v-else @click.stop="$emit('OperationTrigger', 'handleCommand',{actionName:'Comment',mId:item2.mId,subject:item2.subject})">
                                <i class="iconfont icon-dot"></i>
                            </div>
                        ...
                        <div class="mailName">
                            <!--发件箱/文件夹：显示收件人-->
                            <span v-if="item2.source=='FMDRAFT'||item2.source=='FMD'||(item2.source=='FMI'&&item2.mailAddress==item2.fromEx[0].address )">
                                <show-name ref="ShowName" v-if="item2.recipients" :showDetail="item2.recipients[0]" @filterGetMail="filterGetMail" :containAttachment="item2.containAttachment" :longSentDate="DateDisplay(item2.longSentDate)" :tagObjects="item2.tags" :myMap="myMap" :custId="custId"></show-name>
                            </span>

                            <!--收件箱：显示发件人-->
                            <span v-else>
                                <show-name ref="ShowName" v-if="item2.fromEx" :showDetail="item2.fromEx[0]" @filterGetMail="filterGetMail" :containAttachment="item2.containAttachment" :longSentDate="DateDisplay(item2.longSentDate)" :tagObjects="item2.tags" :myMap="myMap" :custId="custId"></show-name>
        ...
        <!-- 高级搜索，组件 -->
        <advanced-Search ref="AdvancedSearch" :dynamicTags="dynamicTags" @AdvancedSearchClick="AdvancedSearchClick"></advanced-Search>
...
export default {
    ...
    created() {
        this.pageGet()
        ep.tail('reloadList', function (data) {
            this.filterGetMail('', '', '', '', 'filterType')
        })
        ep.tail('MailDelayStateGet', (dataObj) => { // 用于定时、延迟发送的邮件通知
            this.filterGetMail('', '', '', '', '')
        })
    },
    methods: {
        ...
        pagesizeSting() {
            ...
            this.$http.post(this.Global.baseURL + this.Global.api.SystemSet.mailset.setindex.updateOptions, data).then(function (res) {
                if (res.body.code.toString() == this.Global.RES_OK) {
                    // console.log('保存邮件条数成功')
                }
            })
        },
        pageGet() {
            this.$http.get(this.Global.baseURL + this.Global.api.SystemSet.mailset.setindex.GetOptions, {
                params: {
                    itemCode: 'MailCount'
                }
            }).then(function (res) {
        ...
        ListSelection() {
            ...
            if (this.listCenterData[this.checkgroupIndex] == undefined) {
                return
            }
            if (this.listCenterData[this.checkgroupIndex].list == undefined) {
                return
            }
            if (this.listCenterData[this.checkgroupIndex].list.length <= 0) {
                return
            }
        ...
        // 日期显示
        DateDisplay(times) {
            var timeFormat = this.$moment(new Date(times)).format('YYYY/MM/DD HH:mm:ss')
            var curData = new Date()
            var timArry = timeFormat.split('/')

            if (timArry[0] != curData.getFullYear()) { // 年份不同返回年
                return timArry[0]
            } else {
                return this.formartTime(timeFormat)
            }
        },
        // 实时计算邮件列表高
        ...
        ListCenterShowDetail(item, index1, index2) {
            ...
            if (this.mailAddress != '') {
                this.isShow = true
                this.windowMode = this.iscustTable != true
                console.log(this.$refs.sliderMailDetail)
                this.$refs.sliderMailDetail.show(item.mId)
                return
            }
            ...
            if (item.status != 1 && (this.subordinateCtid == this.company.ctId || this.subordinateCtid == 0)) { // 未阅读
                item.status = 1
        ...
        // 过滤获取列表|刷新标签
        filterGetMail(attach, color, readFlag, labelName, actionType, Flip) {
        ...
        async onePage(data1, data2) {
            ...
            try {
                if (_this.subordinateCtid != _this.company.ctId && _this.subordinateCtid != 0) {
                    data1.ctid = _this.subordinateCtid
                    data2.ctid = _this.subordinateCtid
                } else {
                    delete data1.ctid
                    delete data2.ctid
                }
                ...
                let ResultData = {}
                _this.target = 'folder'
                if (_this.SearchSelect == 'c') {
                    _this.ParameterConversion(data1)
                    _this.ParameterConversion(data2)
                }
                delEmptyStrObj(data1, true)
                delEmptyStrObj(data2, true)
                var p0 = new Promise((resolve, reject) => {
                    _this.$http.get(_this.Global.baseURL + _this.Global.api.Mail.getMaillist, {
                        params: data1,
                        before(request) {
                ...
                Promise.all([p0, p1]).then(function (results) {
                    ...
                    if (results[0].totalNum > 0) {
                        if (_this.RefreshOrFlip == 'F') {
                            _this.$emit('ShowDetail', results[0].list[0].mId, _this.selectedBoxId.id)//
                            _this.curmId = results[0].list[0].mId
                        }
                        ResultData2 = results[0].list // _this.TagsOjectList(results[0].list)
                        arry = [{
                            'dates': _this.$t('mxpcweb.mail.1528941835031'), // "置顶邮件",
                            'list': ResultData2
                        }]
                    }
                    if (results[1].totalNum > 0) {
                        if (results[0].totalNum == 0 && _this.RefreshOrFlip == 'F') {
                            _this.$emit('ShowDetail', results[1].list[0].mId, _this.selectedBoxId.id)//
                            _this.curmId = results[1].list[0].mId
                        }
                        ResultData = results[1].list//  _this.TagsOjectList(results[1].list)
                        BoxList = _this.dataGrouping(ResultData)
                    ...
                    if (results[0].totalNum > 0) {
                        for (let i = 0; i < ResultData2.length; i++) {
                            _this.listALL.push(ResultData2[i].mId)
                        }
                    }
                    if (arry.length <= 0) {
                        _this.noDataSurce()
                    } else {
                        _this.total = results[1].totalNum
                        _this.noData = false
                        _this.listCenterData = arry
                        _this.PageRefresh(ResultData)
                    }
                }).catch(function (err) {
        ...
        getMaillist(data) {
            ...
            Promise.all([p0]).then(function (results) {
                ...
                _this.loading = false
                _this.loaded = true
                let ResultData = isObject(results[0]) ? results[0].list : [] // _this.TagsOjectList(results[0].list)

                if (ResultData.length <= 0) {
                    _this.noDataSurce()
                } else {
                    _this.total = results[0].totalNum
                    _this.noData = false
                    _this.listCenterData = _this.dataGrouping(ResultData)
                    _this.$emit('ShowDetail', _this.listCenterData[0].list[0].mId, _this.selectedBoxId.id)
                    _this.curmId = _this.listCenterData[0].list[0].mId
                    _this.PageRefresh(ResultData)
                }
            }).catch(function (params) {
                _this.noDataSurce()
                _this.$message.error(params)
            })
        },
        // 获取未分发邮件
        UndistributedMail(data) {
            ...
            Promise.all([p0]).then(function (results) {
                ...
                let ResultData = results[0].list // _this.TagsOjectList(results[0].list)
                if (ResultData.length <= 0) {
                    _this.noDataSurce()
                } else {
                    _this.total = results[0].totalNum
                ...
                .catch(function (params) {
                    _this.noDataSurce()
                    _this.$message.error(_this.msg(params.body))
                })
        },
        // 获取所有置顶邮件
        TopMail(data) {
            let _this = this
            _this.target = 'toplist'
            data = this.InitializationLoading(data)
            var p0 = new Promise((resolve, reject) => {
                _this.$http.get(_this.Global.baseURL + _this.Global.api.Mail.getMaillist, {
                    params: data
        ...
        // 快速搜索
        SearchClick(keywords, RefreshOrFlipT, isFirst, type) {
        ...
        formartTime(time, option = {}) {
            ...
            // 传入时间的年月日
            let t = this.$moment(time).format('YYYY-MM-DD HH:mm:ss')
            let date = new Date()

            // 判断一年以内
            // 前一年时间
            let beforeYear = parseInt(this.$moment(date).format('YYYY')) - 1 + this.$moment(date).format('-MM-DD')
            beforeYear = this.$moment(beforeYear).format('YYYY-MM-DD')
            // 后一年
            // let afterYear = parseInt(this.$moment(date).format('YYYY')) + 1 + this.$moment(date).format('-MM-DD')
            // 前两天的时间
            let beforeDay2 = this.$moment(date).format('YYYY-MM-') + (parseInt(this.$moment(date).format('DD')) - 2)
            // 传入日期偏差
            let offsetDay = parseInt(this.$moment(date).format('DD')) - parseInt(this.$moment(t).format('DD'))

            // 传入的时间是否大于1年前今天的时间，并且小于前两天的时间。那就是过去一年内的时间
            if (new Date(t) * 1 > new Date(beforeYear) * 1 && new Date(t) * 1 < new Date(beforeDay2) * 1) {
                return this.$moment(time).format('MM/DD HH:mm')
            } else if (offsetDay === 0) { // 今天
                return today + ' ' + this.$moment(t).format('HH:mm')
            } else if (offsetDay === 1) { // 昨天
                return yesterday + ' ' + this.$moment(t).format('HH:mm')
            } else if (offsetDay === -1) { // 明天
                return tomorrow + ' ' + this.$moment(t).format('HH:mm')
            } else if (offsetDay === -2) { // 后天
                return aftertomorrow + ' ' + this.$moment(t).format('HH:mm')
            } else { // 都不在时
        ...
        ParameterSplicing(form) {
            ...
            if (this.target == 'folder') {
                ...
                this.SearchClick(this.keywords, this.RefreshOrFlip, false, this.SearchType)
    ...
    watch: {
        selectedBoxId: {
            handler(curVal, oldvalue) {
                ...
                if (this.target != 'Quicklist' && this.target != 'advancedlist') {
                    this.filterType = 'all'
                    this.attach = ''
                    this.color = ''
                    this.readFlag = ''
