    ...
    <div class="ConditionEdit">
        <template>
            <div>
                ...
                <con-item ref="conItem" :modules="modules" @add="addCondition()" v-for="(cItem,cIndex) in conditions" :key="cItem.uuid" :isLast="cIndex==conditions.length-1" :index="cIndex" :conLength="conditions.length" @remove="removeCondition(cIndex)" :logic="logicFlags[cIndex]" @changeLogic="changeLogic(cIndex)"></con-item>
                </el-radio-group>
            </div>
        </template>
    </div>
