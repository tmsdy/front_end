<template>
    <div class="ConditionEdit">
        <template>
            <div>
                <con-item ref="conItem" :modules="modules" v-for="(cItem,cIndex) in conditions" :condition="cItem" :key="cIndex" :isLast="cIndex==conditions.length-1" :index="cIndex" :conLength="conditions.length" :logic="logicFlags[cIndex]" @changeLogic="changeLogic(cIndex)" @clearQuick="clearQuick"></con-item>
            </div>
        </template>
    </div>
</template>
...
export default {
    ...
    methods: {
        clearQuick() {
            this.$emit('clearQuick')
        },
        clearChildren() {
            this.$refs.conItem[0].clear()
        },
        clearConditions() {
            this.conditions = [{
                conditionFirst: { time: false },
