<template>
    <div class="addName MXscroll">
        <!-- 接收人 -->
        <!-- 新建接收人列表 -->
        <page-detail :title="$t('mxpcweb.am.1542012292798')" iconfont="icon-personnel" :detailTitle="$t('mxpcweb.am.1542012644547')" @toList="$emit('tabDataCheck','1')"></page-detail>
        <template>
            ...
            <div class="list">
                <template v-if="tabData=='0'">
                    ...
                    <div style="padding:10px 0;">
                        ...
                        <input type="file" id="sendeeBox_excel" @change="excel($event)" style="position: absolute;display: none;" accept=".xlsx">
                        <!-- 浏览 -->
                ...
                <template v-if="tabData=='1'">
                    ...
                    <div style="padding:10px 0;">
                        <el-row :gutter="20">
                            <el-col :span="7">
                                <!-- 邮箱 -->
                                <div style="padding:5px 30px 5px 0;">{{$t('mxpcweb.am.1531900664136')}}</div>
                            </el-col>
                            <el-col :span="7">
                                <!-- 昵称 -->
                                <div style="padding:5px 30px 5px 0;">{{$t('mxpcweb.am.1531900824830')}}</div>
                            </el-col>
                            <el-col :span="7">
                                <!-- 手机 -->
                                <div style="padding:5px 30px 5px 0;">{{$t('mxpcweb.am.1531901900133')}}</div>
                            </el-col>
                        </el-row>
                        <el-form onsubmit="return false" label-width="0" ref="ruleForm" :model="formData">
            ...
            <div class="list" style="font-size:14px;">
                <!-- 确认导入 -->
                <el-button v-if="tabData=='0'" type="primary" :loading="importLoading" @click="importFiles()" size="small">{{$t('mxpcweb.am.1531901919892')}}</el-button>
                <!-- 确认添加 -->
...
export default {
    ...
    data() {
        ...
        return {
            ...
            rulesMailVal: [
                // 请输入正确的邮箱地址
                { type: 'email', message: this.$t('mxpcweb.am.1531902516260'), trigger: 'blur' }
            ],
            rulesPhone: [
                // 请输入正确的手机号
    ...
    methods: {
        importFiles() {
            ...
            fd.append('invokeName', _this.detailData.invokeName)
                ...
                if (res.body.code.toString() == this.Global.RES_OK) {
                    ...
                    setTimeout(() => {
                        _this.$emit('tabDataCheck', '3', {
                            invokeName: _this.detailData.invokeName,
        ...
        async excel(e) { // 导入excel解析
            if (e.target.accept != '.xlsx') {
                _this.$message.error(this.$t('mxpcweb.am.1540975854934'))
                return
            }
