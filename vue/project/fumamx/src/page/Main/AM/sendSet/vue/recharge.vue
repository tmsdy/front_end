<template>
    <div class="recharge" v-loading="loading">
        <!-- 发送设置 -->
        <!-- 充值 -->
        <page-detail style="z-index:2;" :title="$t('mxpcweb.am.1531903938449')" iconfont="icon-setting" :detailTitle="$t('mxpcweb.am.1531904952432')" @toList="$emit('backPage', '0')"></page-detail>
        <div class="list1">
            <!-- 充值 -->
        ...
        <div class="list2">
            <!-- 账户余额: -->
            {{$t('mxpcweb.am.1532511326836')}} ¥ {{parseInt(avaliableBalance).toFixed(2)}}
        </div>
        <div class="list3">
            <div>
                <!-- 在线充值 -->
            ...
            <template v-if="radio=='2'">
                ...
                <el-row class="list">
                    ...
                    <el-col :span="7">
                        ...
                        <input ref="numCount" style="width:20px;opacity:0;" type="text" value="11006935635401" class="contents">
                        <!-- 复制 -->
                        <el-button size="mini" @click="Copy('numCount')">{{$t('mxpcweb.am.1531893077381')}}</el-button>
                    </el-col>
                </el-row>
                <div class="Tip">
                    <!-- 线下汇款处理说明 -->
        ...
        <template v-if="radio=='1'">
            ...
            <div class="list6" style="margin-top:20px;">
                <!-- 确定 -->
        ...
        <!-- 支付确认 -->
        <el-dialog :close-on-click-modal="false" v-dialogDrag :title="$t('mxpcweb.am.1532586521149')" :visible.sync="payDialog" custom-class="width520" :closeOnClickModal="false" class="payDialog" :modal-append-to-body="false">
            <div class="payBox">
        ...
        lipayCall() {
            ...
            this.$http.get(this.Global.baseURL + this.Global.api.v2.payCenter_onlinepay, {
                ...
                if (res.body.code.toString() === this.Global.RES_OK) {
                    newUrl.location = 'https://openapi.alipay.com/gateway.do?' + res.body.data.resultString // 后更改页面地址
                    this.payDialog = true
