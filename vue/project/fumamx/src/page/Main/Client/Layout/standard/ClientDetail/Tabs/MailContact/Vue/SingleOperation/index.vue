<template>
    <div class="SingleOperation">
        <!-- 归并 -->
        <!-- <span :title="$t('mxpcweb.mail.1528702678326')" @click.stop="$emit('OperationTrigger', 'MaileMerge',{mId:DetailData.mId},[DetailData.mId])">
            <i class="iconfont icon-filed"></i>
        </span> -->
        <!-- 内分发 -->
        <span v-if="selectedBoxId.target!='undistributed'" :title="$t('mxpcweb.mail.1528702683332')" @click.stop="$emit('OperationTrigger','DialogInDistribute',[DetailData.mId])">
            <i class="iconfont icon-inside"></i>
        </span>
        <!-- 移动 -->
        <!-- <span :title="$t('mxpcweb.mail.1528702683623')" @click.stop="$emit('OperationTrigger','openMovingFolders',[DetailData.mId],1)">
        ...
        <span :title="$t('mxpcweb.mail.1528708499750')" @click.stop="">
            <el-dropdown trigger="click" @command="handleCommand">
                ...
                <el-dropdown-menu slot="dropdown" style="margin-top:7px;">
                    ...
                    <el-dropdown-item :command='{actionName:"AttachmentsSend",subject:DetailData.subject,size:DetailData.size,rawData:DetailData.rawData}'>{{$t('mxpcweb.mail.1528702540482')}}</el-dropdown-item>
