<template>
  <div class="customerItem">
    <div class="cover" v-if="isHS() && workOrderItem.isDefault == '1'">
        <!-- 封面 -->
        <span class="coverName">默认</span>
    </div>
        ...
        <div class="grid-content bg-purple">
          ...
          <div v-else class="grid-content bg-purple" style="display:inline-block;width:100%">
            <span class="ellipsis" v-for="(fieldItem,fieldIndex) in listSet.listSetAllow" :key="fieldIndex" :class="classItem(fieldItem)" @click="listClick(fieldItem, workOrderItem)" style="display:inline-block;width:16%;text-align:left;height:17px;" :style="{'width':(parseInt(fieldItem.columnWidth)/listSet.allColumnWidth)*99+'%'}">
              <component :list="workOrderItem" :class="detailOpt.length > 0 && fieldIndex > 1 && fieldIndex > listSet.listSetAllow.length - 2 ? 'listTimes1' : ''" :workOrderItem="workOrderItem" v-bind:is="'control'+fieldItem.controlTypeId" ref="control" :itemData="fieldItem" :moduleCode="moduleCode" :accountInfo="workOrderItem.accountInfo" :value="{value:fieldItem.fieldName?workOrderItem[fieldItem.fieldName]:''}"></component>
            </span>
          </div>
        </div>
      </el-col>
