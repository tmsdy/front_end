...
function createReactiveObject(
    ...
    if (!isObject(target)) { // 只能是对象才能搞成响应式？
        if (__DEV__) {
    ...
    const handlers = collectionTypes.has(target.constructor) //存在Set, Map, WeakMap, WeakSet这种的
        ? collectionHandlers
        : baseHandlers // 一般走这儿
    observed = new Proxy(target, handlers) //把target变成响应式
