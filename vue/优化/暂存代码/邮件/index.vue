  ...
  <div class="ListCenter" v-loading="loading" :style="{width:( custId==''&&mailAddress==''?'380px':'100%')}">
    ...
    <el-checkbox-group v-model="checkedListAll" @change="handleCheckedCitiesChange">
      <div class="mailList MXscroll" ref="mailList" :style="{'padding-bottom':(custId==''&&mailAddress==''?'25px':'47px')}">
        <virtual-list class="MXscroll" :size="60" :remain="remain" :start="0" :item="ListItem" :variable="getVariableHeight" :itemcount="listCenterData.length" :itemprops="getItemProps" />
      </div>
    </el-checkbox-group>
    <!-- 邮件列表翻页 -->
  ...
  methods: {
    getItemProps(itemIndex) {
      return {
        ...
        props: {
          index2: itemIndex,
          item2: this.listCenterData[itemIndex] || {},
          custId: this.custId,
          checkedListAll: this.checkedListAll,
          mailAddress: this.mailAddress
        }
      }
    },
    getVariableHeight(itemIndex) {
      return this.listCenterData[itemIndex].dates ? 29 : 82
    },
        ...
        for (let i = 0; i < Data.length; i++) {
          ...
          if (i != Data.length - 1) { // 不是最后一条
            ...
            if (timeFormat != timeFormat2) {
              ...
              checkgroupIndex += 1
              list.forEach(item => {
                item.index = checkgroupIndex
              })
              Arry = [...Arry, { 'dates': timeFormat }, ...list]
              list = []
            }
          } else {
            // Arry.push({
